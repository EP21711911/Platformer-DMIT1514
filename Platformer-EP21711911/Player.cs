...
private const int _Speed = 150, _JumpForce = -100;
    ...
    internal void Initialize()
    {
        ...
        _dimensions = new Vector2(33, 48);
        _facingRight = true;
    }
    internal void LoadContent(ContentManager content)
    {
        _idleSequence = new CelAnimationSequence(content.Load<Texture2D>("Idle"), 48, 1 / 8f);
        _walkSequence = new CelAnimationSequence(content.Load<Texture2D>("Walk"), 48, 1 / 8f);
        _jumpSequence = new CelAnimationSequence(content.Load<Texture2D>("Jump"), 48, 1 / 8f);
    }
